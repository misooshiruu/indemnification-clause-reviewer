...
- INTERNAL CONSISTENCY: an edit must never leave the clause self-contradictory. If moving a lever conflicts with other language ALREADY in the clause that speaks to the same point, you MUST also amend that other language in the same response so the two no longer fight. Prefer fixing the conflicting text in place over adding an overriding sentence on top of it. Example: if you carve the indemnity out of the liability cap, you must also delete or revise any phrase that still subjects the indemnity to that cap — e.g. remove "including its indemnification obligations under this Section 9" from the cap sentence — rather than only adding a new "notwithstanding the cap" sentence while the contradictory inclusion remains. These consistency fixes belong to the lever you are already moving and do NOT count as touching another lever.
